...
function getPriceLines(form: FormState) {
  ...
  if (form.sofaSeats > 0) {
    ...
    lines.push({ label: `Sofa — ${form.sofaSeats} seat${form.sofaSeats > 1 ? "s" : ""}`, amount: amt });
  }
  if (form.recliners > 0)
  ...
  if (form.carpetSqft > 0) {
    ...
    lines.push({ label: `Carpet — ${form.carpetSqft} sq ft`, amount: Math.max(500, form.carpetSqft * rate) });
  }
  if (form.carType) {
...
function CounterRow({ label, subLabel, value, onChange }: {
  ...
  return (
    <div className="flex items-center justify-between py-0.5">
      ...
      <div className="flex items-center gap-3 flex-shrink-0">
        ...
        <span className="w-5 text-center font-bold text-gray-900 tabular-nums">{value}</span>
...
export default function BookingForm({ showHeader = true }: { showHeader?: boolean }) {
  ...
  const blockedMinutes = new Set(
    blockedSlots.map((s) => { const [h, m] = s.split(":").map(Number); return h * 60 + m; })
  );
  ...
  useEffect(() => {
    if (!form.date) { setBlockedSlots([]); return; }
  ...
  const handleDateChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (selected < BOOKING_OPENS) return;
  ...
  const handleSubmit = async () => {
    ...
    if (!(form.sofaSeats > 0 || form.carpetSqft > 0 || form.carType !== ""))
    ...
    return (
      <div className="w-full max-w-lg mx-auto py-8">
        <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-8 text-center">
          <div className={`w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-5 ${isPaid ? "bg-green-50" : "bg-amber-50"}`}>
            {isPaid ? (
          ...
          <h2 className="text-xl font-bold text-gray-900 mb-1">{isPaid ? "Booking Confirmed!" : "Request Received!"}</h2>
          ...
          <div className="bg-gray-50 rounded-2xl p-4 mb-6 text-left">
            ...
            <p className="font-mono font-bold text-gray-900 text-base tracking-wide">{submitted.bookingId}</p>
          </div>
          <div className="flex justify-between text-sm text-gray-600 mb-1">
            <span>Total</span>
            <span className="font-semibold text-gray-900">₹{submitted.price.toLocaleString("en-IN")}</span>
          </div>
          <div className="flex justify-between text-sm text-gray-600 mb-4">
            <span>Status</span>
            <span className={`font-semibold ${isPaid ? "text-green-600" : "text-amber-600"}`}>{submitted.paymentStatus}</span>
          </div>
          <p className="text-xs text-gray-400 mb-6">Screenshot this for your records.</p>
          <button onClick={() => setSubmitted(null)} className="w-full border border-gray-200 text-gray-700 py-3 rounded-xl text-sm font-medium hover:bg-gray-50 transition">
    ...
    <div className="w-full max-w-lg mx-auto space-y-4">
      {showHeader && (
        <div className="text-center pb-1">
          <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-2">CleanWarks Bangalore</p>
          <h1 className="text-[26px] font-bold text-gray-900 leading-tight">Book a Cleaning</h1>
          <p className="text-sm text-gray-500 mt-1.5">Sofa · Carpet · Car seats · At your doorstep</p>
        </div>
      )}
      ...
      <Card step="02" title="Service Details">
        <div className="inline-flex items-center gap-2 bg-gray-900 text-white px-3.5 py-2 rounded-xl text-sm font-medium">
          <span>🛋️</span><span>Sofa Cleaning</span>
        </div>
        ...
        <div className="flex items-center justify-between py-0.5">
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            <input type="number" name="carpetSqft" value={form.carpetSqft || ""} placeholder="0" min={0}
              onChange={(e) => handleCounter("carpetSqft", parseInt(e.target.value) || 0)}
        ...
        <div>
          ...
          <select name="carType" value={form.carType} onChange={handleChange} className={inputCls}>
      ...
      <Card step="03" title="Your Address">
        <input name="address" placeholder="Flat/house, building, street, area" value={form.address} onChange={handleChange} className={inputCls} />
        <input name="mapsLink" placeholder="Google Maps link (optional but helpful)" value={form.mapsLink} onChange={handleChange} className={inputCls} />
        <p className="text-xs text-gray-400">A Maps pin helps our team arrive faster.</p>
      </Card>

      <Card step="04" title="Pick a Date & Time">
        <input type="date" name="date" value={form.date} min={minDate} onChange={handleDateChange} className={inputCls} />
        {form.date && (
          <div className="space-y-3 pt-1">
            {slotsLoading ? (
              ...
              <p className="text-sm text-red-500 py-1">No slots available for this date. Please pick another day.</p>
            ) : (
              <>
                <p className="text-xs text-gray-400">
                  {availableCount} slot{availableCount !== 1 ? "s" : ""} available
                  {priceLines.length > 0 && <span className="ml-1">· {duration % 1 === 0 ? duration : duration.toFixed(1)}h per booking</span>}
                </p>
                  ...
                  {displayableSlots.map((slot) => {
                    ...
                    return (
                      <button key={slot} type="button" disabled={!available}
                        ...
                        className={["py-2.5 rounded-xl text-xs font-medium transition select-none",
                          !available ? "bg-gray-100 text-gray-300 line-through cursor-not-allowed"
                ...
                {availableCount === 0 && (
                  <p className="text-sm text-red-500 text-center pt-1">All slots are booked for this date. Please try another day.</p>
                )}
              </>
            )}
          </div>
      ...
      <Card step="05" title="Price Summary">
        {priceLines.length === 0 ? (
          <p className="text-sm text-gray-400 py-1">Add items above to see your price estimate.</p>
        ) : (
          <div className="space-y-2.5">
            {priceLines.map((line) => (
              <div key={line.label} className="flex justify-between text-sm">
                <span className="text-gray-600">{line.label}</span>
                <span className="font-medium text-gray-900">₹{line.amount.toLocaleString("en-IN")}</span>
              </div>
            ))}
            <div className="h-px bg-gray-100" />
            ...
            <div className="flex justify-between text-sm">
              <span className="text-emerald-700 font-medium">Advance (pay now)</span>
              <span className="text-emerald-700 font-bold">₹{ADVANCE.toLocaleString("en-IN")}</span>
            </div>
      ...
      <Card step="06" title="Advance Payment">
        <div className="bg-amber-50 border border-amber-100 rounded-xl p-4">
          <p className="text-sm font-semibold text-amber-900">₹250 advance required to confirm your slot</p>
          <p className="text-xs text-amber-700 mt-0.5 leading-relaxed">This is adjusted in your final bill. Pay via UPI and paste the transaction ID below.</p>
        </div>
        <div className="flex flex-col items-center gap-2 py-1">
          <img src="/upi-qr.png" alt="Scan to pay ₹250 advance" className="w-44 h-44 rounded-2xl border border-gray-100" />
          <p className="text-xs text-gray-400">Scan with any UPI app to pay</p>
        </div>
        <div className="flex items-center justify-between bg-gray-50 border border-gray-100 rounded-xl px-4 py-3">
          <div>
            <p className="text-[11px] text-gray-400 font-medium uppercase tracking-wide mb-0.5">UPI ID</p>
            <p className="font-mono font-bold text-gray-900 text-sm select-all">{UPI_ID}</p>
          </div>
          <button type="button" onClick={handleCopyUPI}
            className={["text-xs font-medium px-3 py-1.5 rounded-lg border transition",
              copied ? "bg-green-50 border-green-200 text-green-700" : "bg-white border-gray-200 text-gray-500 hover:border-gray-300",
            ].join(" ")}>
            {copied ? "Copied ✓" : "Copy"}
          </button>
        </div>
        <input name="upiTransactionId" placeholder="UPI Transaction ID (e.g. 407812345678)" value={form.upiTransactionId} onChange={handleChange} className={inputCls} />
        <p className="text-xs text-gray-400">Haven&apos;t paid yet? You can still submit — our team will call to collect the advance before confirming your slot.</p>
      </Card>

      <textarea name="notes" placeholder="Special instructions — fabric type, stains, access details, etc." value={form.notes} onChange={handleChange} rows={3} className={inputCls} />
      ...
      <button onClick={handleSubmit} disabled={loading}
        ...
        ) : `Confirm Booking${priceLines.length > 0 ? ` — ₹${price.toLocaleString("en-IN")}` : ""}`}
      </button>

      <p className="text-center text-xs text-gray-400 pb-6">Free cancellation up to 2 hours before the service.</p>
    </div>
  );
}
